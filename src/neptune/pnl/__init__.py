...
def unrealised_pnl(lots: list[Lot], current_price: float, direction: int) -> float:
    """Mark-to-market on open lots: qty * (current - entry) * direction."""
    return sum(
        lot.quantity * (current_price - lot.entry_price) * direction for lot in lots
    )
...
def day_pnl(
    ...
    total = 0.0
    for lot in lots:
        ref = lot.entry_price if as_of is not None and lot.entry_date == as_of else prev_close
...
def reduce_position(
    ...
    Returns the remaining open lots and the realised P&L of the reduction. Does not
    mutate the input. Realised per matched share = (exit - entry) * direction.
    """
    if quantity <= 0:
    ...
    if method is CostBasisMethod.AVCO:
        return _reduce_avco(lots, quantity, exit_price, direction)
    if method is CostBasisMethod.SPECIFIC:
        if specific_index is None:
            raise ValueError("SPECIFIC method requires specific_index")
        return _reduce_specific(lots, quantity, exit_price, direction, specific_index)
    return _reduce_fifo(lots, quantity, exit_price, direction)


def _reduce_fifo(lots, quantity, exit_price, direction):
    remaining: list[Lot] = []
    realised = 0.0
    to_close = quantity
    for lot in lots:  # oldest first (insertion order)
        ...
        realised += take * (exit_price - lot.entry_price) * direction
        to_close -= take
...
def _reduce_specific(lots, quantity, exit_price, direction, specific_index):
    ...
    realised = quantity * (exit_price - lot.entry_price) * direction
    remaining = list(lots)
...
def _reduce_avco(lots, quantity, exit_price, direction):
    total_qty = open_quantity(lots)
    avg_cost = sum(lot.quantity * lot.entry_price for lot in lots) / total_qty
    realised = quantity * (exit_price - avg_cost) * direction
    leftover = total_qty - quantity
    if leftover <= 1e-12:
        return [], realised
    # Collapse remaining holding to a single lot at the average cost, keeping the
    # earliest entry date.
    earliest = min(lot.entry_date for lot in lots)
    return [Lot(quantity=leftover, entry_price=avg_cost, entry_date=earliest)], realised
