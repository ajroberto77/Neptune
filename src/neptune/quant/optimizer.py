...
from neptune.quant.factors import FACTORS, MARKET_FACTOR, STYLE_FACTORS
...
class Candidate:
    ...
    ``idio_var`` is the name's IDIOSYNCRATIC (factor-residual) daily variance — the diagonal of
    the factor-model covariance Σ = BᵀFB + D. It is used only by the covariance objective
    (``factor_cov`` passed to the optimizer); when None it falls back to ``variance``."""
...
def compute_residual(
    ...
    ``factors`` is the known factor set whose residual exposures are accumulated (loadings keyed
    outside it are ignored). Defaults to the FF5+MOM ``FACTORS``; the caller passes an extended
    set when monitor factors have been PROMOTED into the neutralized model."""
    if long_aum <= 0:
...
def optimize_hedge(
    ...
    hedge_factors: tuple[str, ...] = HEDGE_FACTORS,
    ...
    ``factor_cov`` (when given) switches the objective to net-book minimum variance.
    """
    ...
    weights, status = _solve_qp(
        ...
        beta_add_budget=beta_add_budget, hedge_factors=hedge_factors, factor_cov=factor_cov,
    )
    ...
    return _build_proposal(
        ...
        beta_tol=beta_tol, sector_limit=sector_limit, hedge_factors=hedge_factors,
    )
...
def complexity_frontier(
    ...
    if caps is not None:
        ...
        k = _min_neutralizing_cap(
            ...
            hedge_factors=hedge_factors, factor_cov=factor_cov,
        )
...
def _solve_qp(
    ...
    """Minimize tracking error to the residual (or, with ``factor_cov``, the net-book variance).
    With ``hard``, the beta/factor limits (and, when given, the per-sector concentration cap) are
    constraints; otherwise only the per-name size ceiling is enforced (the limits live in the
    objective), guaranteeing a feasible solution. ``beta_add_budget`` (when set) fences
    negative-beta shorts (see below). ``hedge_factors`` is the neutralized style+promoted set."""
    ...
    if factor_cov is not None and hard:
        # NET-BOOK MINIMUM-VARIANCE objective with a factor-model covariance Σ = BᵀFB + D.
        # Applied only to HARD solves (the approvable proposal), where neutrality is a constraint
        # and the covariance shapes the basket WITHIN the feasible set — the true min-variance
        # hedge. The SOFT frontier/ranking keep the tracking-error objective: their job is to find
        # how many names REACH neutrality, a tracking question, not a within-feasible-set risk one.
        # net_full is the net book's exposure over [MKT, *hedge_factors]; quad_form(net_full, F)
        # is the net book's factor-space variance and Σ dᵢxᵢ² is the hedge's diversifiable
        # (idiosyncratic) variance — together the net book's daily-return variance (the constant
        ...
        net_full = cp.hstack([net_beta, net_factors])
        objective = cp.Minimize(
            cp.quad_form(net_full, cp.psd_wrap(factor_cov)) + idio @ cp.square(x)
        )
    else:
        ...
        objective = cp.Minimize(
            cp.square(net_beta) + cp.sum_squares(net_factors) + diversification
        )
...
def _size_capped(
    ...
    return _build_proposal(
        ...
        beta_tol=beta_tol, sector_limit=sector_limit, n_cap=n_cap, hedge_factors=hedge_factors,
    )
