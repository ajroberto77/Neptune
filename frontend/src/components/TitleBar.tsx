...
// (assets/brand/Neptune - Glyph White.svg). The exported viewBox is a padded 148x148 square,
// but the mark's actual ink only spans x:38.23-109.77 y:29-119 (measured via the path's
// getBBox() in a real browser, not eyeballed) — leaving the export viewBox in place renders
// the glyph at ~40% of its box and it reads as too small. Cropped to the ink bounds (rounded
// outward a hair for curve overshoot: 38 29 72 90) so the box and the mark match, sized to
// Mercury's title-bar glyph spec: 17px painted height, width derived from that crop's aspect
// ratio (72:90 = 0.8 -> round(17 * 0.8) = 14px), 11px to the wordmark. currentColor + color on
// the class (not a hardcoded fill) so it follows the surrounding text like the wordmark does.
function Glyph() {
  return (
    <svg
      className="mr-[11px] h-[17px] w-[14px] flex-shrink-0 text-white"
      viewBox="38 29 72 90"
      fill="none"
...
export function TitleBar({
  ...
  return (
    <div className="app-drag flex h-12 flex-shrink-0 items-center gap-3 border-b border-ocean-border bg-ocean-panel pl-4">
      {/* Glyph-to-wordmark spacing is the glyph's own margin-right (11px, exact spec), not the
          row's gap-3 — grouped in its own flex box so the two don't stack. */}
